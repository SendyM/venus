...
public class Constants {
	...
	/**
	 * AJAX_SESSION_TIMEOUT主要用在ajax session timeout过滤器中
	 * 参考http的status的标准状态吗408代表超时 ！！！注意在js页面中注册全局的ajax处理代码 配合使用
	 */
	...
	/**
	 * CURRENT_USER 代表session中代表当前用户的key值 
	 * session中得到的是SysUser对象 
	 */
	public static final String CURRENT_USER = "sysUser";
	/**
	 * CURRENT_USER_NAME 代表session中代表当前用户POJO的key值 
	 * session中得到的是username String对象
	 */
	public static final String CURRENT_USER_NAME = "sysUserName";
	
	/**
	 * 登录验证失败存储的key值 
	 * 同FormAuthenticationFilter.DEFAULT_ERROR_KEY_ATTRIBUTE_NAME一样
	 * 
	 */
	 ...
	 * 这里默认关闭 
	 * 配置文件里的配置会覆盖此配置 
	 * 请不要在这里修改 如需修改请改config.properties文件
	 */
	public static  boolean CAPTCHA_ENABLED = true;
	/**
	 * 是否开启验证码校验的request域中attr的key值
	 * 前台页面可以根据该属性的值选择是否显示验证码
	 * 不允许修改
	 */
	...
	/**
	 * 把session放入到加密请求的request域中的key 
	 * value是Session对象
	 */
	public static final String GET_SESSION_BY_TOKEN = "getSessionByToken";
	
	/**
	 * 存储后台管理界面的url 同时也是登陆成功后访问的url 
	 * 请和shiro相关配置中的shiro.success.url保持一致
	 * 请不要在这里修改
	 * 配置文件中如果配置错误系统启动会停止
	 */
	 ...
	 * 默认值 配置文件里如果有会覆盖此值
	 */
	public static  String SYS_COPYRIGHT = "www.myweixin.club";
	/**
	 * 系统名称
	 * 默认值 配置文件里如果有会覆盖此值
	 */
	public static  String SYS_NAME = "venus system";
	 ...
	 * 系统超时
	 */
	private static final String TIME_OUT="0007";
	/**
	 * 错误代码表
	 */
	public static Map<String,String> ERROR_CODE_MAP=Maps.newHashMap();
	static{
