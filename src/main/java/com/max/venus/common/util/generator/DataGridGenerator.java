package com.max.venus.common.util.generator;

import com.max.venus.common.annotation.Th;
...
/**
 * 用于生成easyui的datagrid数据表格
 * @author Sendy
 * @since 2016年11月7日 下午2:59:36
 * @version V1.0
 * 
 */
public class DataGridGenerator {
	...
	public static void main(String[] args) throws Exception {
		...
		dataGridGenerator.init();
		//在这里放上你想生成的实体
		dataGridGenerator.process(SysUser.class);
	}
	...
	public void process(Class<?> clazz) throws Exception {
		...
		// 创建模版对象
		Template t = cfg.getTemplate("dataGrid.ftl");
		// 在模版上执行插值操作，并输出到制定的输出流中
		resultMap.put("itemMap",itemMap);
		resultMap.put("dataGridName", clazz.getSimpleName().substring(0, 1).toLowerCase()+clazz.getSimpleName().substring(1));
		resultMap.put("dataGridId", clazz.getSimpleName().substring(0, 1).toLowerCase()+clazz.getSimpleName().substring(1)+"List");
