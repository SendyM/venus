package com.max.venus.common.web.listener;

import com.max.venus.common.config.AppConfig;
import com.max.venus.common.util.PasswordUtils;
		...
		if (event.getApplicationContext().getParent() == null) {
			...
			if (StringUtils.isNotBlank(enableCaptcha)) {
				...
				if (enableCaptcha.equals("1") || enableCaptcha.equals("yes") || enableCaptcha.equals("true")) {
					// 开启
					com.max.venus.common.config.Constants.CAPTCHA_ENABLED = true;
					System.out.println("打开登陆验证码校验");
				} else if (enableCaptcha.equals("0") || enableCaptcha.equals("no") || enableCaptcha.equals("false")) {
					// 关闭
					com.max.venus.common.config.Constants.CAPTCHA_ENABLED = false;
			...
			if (StringUtils.isNotBlank(successUrl)) {
				...
				com.max.venus.common.config.Constants.SUCCESS_URL = successUrl;
			} else {
				// 报错停止启动
			...
			if (StringUtils.isNotBlank(sysName)) {
				...
				com.max.venus.common.config.Constants.SYS_NAME = sysName;
			}
			System.out.println("系统名称:" + com.max.venus.common.config.Constants.SYS_NAME);
			// 读取配置文件 设置系统版权信息
			...
			if (StringUtils.isNotBlank(sysCopyright)) {
				...
				com.max.venus.common.config.Constants.SYS_COPYRIGHT = sysCopyright;
			}
			System.out.println("系统版权:" + com.max.venus.common.config.Constants.SYS_COPYRIGHT);
