...
public class ShiroDbRealm extends AuthorizingRealm {
	 ...
	 *
	 *
	 *从数据库查询出来的账号名和密码,与用户输入的账号和密码对比当用户执行登录时,在方法处理上要实现user.login(token);
	 *然后会自动进入这个类进行认证交给AuthenticatingRealm使用CredentialsMatcher进行密码匹配，
	 *如果觉得人家的不好可以自定义实现,能查到此用户的话把用户的信息封装到该对象中;
	 *SimpleAuthenticationInfo(sysUser,sysUser.getUserPassword().toCharArray(),getName());
	 *第一个参数是数据库查询出来的sysUser（这里肯定不为空,为空时早已经返回null）
	 *第二个参数是数据库查询出来的sysUser的密码,toCharArray()为了安全,String在内存中,
	 *有快照第三个参数是本realm的名字;
	 */
	@Override
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		if (sysUser == null) {
			...
			return new SimpleAuthenticationInfo(
					...
					sysUser,
					// 密码
					sysUser.getUserPassword().toCharArray(),
					// salt
